    ...
    public class SimCarrierTask : SimTaskBase
    {
        ...
        internal void UpdateCurrentSite(ushort poscode)
        {
            ...
            else
            {
                ...
                Track track = PubMaster.Track.GetTrackByCode(poscode);
                if (track != null)
                {
                    SetNowTrack(track);
                }
        ...
        {
            if(MTimer._.IsOver(TimerTag.CarrierResetTime, ID, 20))
            {
                //DevStatus.ActionTime = 0;
                //DevStatus.ActionType = DevCarrierSignalE.复位;
                //DevStatus.TakeTrackCode = 0;
                //DevStatus.GiveTrackCode = 0;
            }
        }
