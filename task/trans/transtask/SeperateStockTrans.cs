    ...
    public class SeperateStockTrans : BaseTaskTrans
    {
        ...
        private void DoMoveGood(StockTrans trans, StockTransDtl dtl, Stock top)
        {
            ...
            if (top != null)
            {
                ...
                {
                    uint transid = _M.AddTransWithoutLock(trans.area_id, 0, TransTypeE.库存转移, top.goods_id, top.id, trans.take_track_id, dtl.dtl_give_track_id, TransStatusE.检查轨道, 0, trans.line, DeviceTypeE.前摆渡);
