...
namespace task.trans.transtask
{
    ...
    public class SameSideInTrans : BaseTaskTrans
    {
        ...
        public override void AllocateDevice(StockTrans trans)
        {
            ...
            if (_M.HaveTrackButSortTransForDown(trans.give_track_id, trans.id))
            {
                #region 【任务步骤记录】
            ...
            {
                #region 【任务步骤记录】
                _M.SetStepLog(trans, false, 1507, string.Format("当前没有空闲的摆渡车"));
                #endregion
                return;
            }
            ...
            switch (track.Type)
            {
                ...
                case TrackTypeE.前置摆渡轨道:
                    ...
                    if (isNotLoad && isStopNoOrder)
                    {
                        ...
                        {
                            #region 【任务步骤记录】
                            _M.SetStepLog(trans, false, 1607, string.Format("砖机[ {0} ]的工位轨道[ {1} ]不满足取砖条件；{2}；",
                            ...
                            if (!isStopNoOrder && carrier.InTask(DevCarrierOrderE.取砖指令))
                            {
                                ...
                                if (carrier.DevAlert.CanNotActionForTaking() && mTimer.IsOver(trans.carrier_id + "LoadError", 20, 10))
                                {
                                    ...
                                    if (carrier.DevStatus.DeviceStatus == DevCarrierStatusE.停止)
                                    {
                                        ...
                                        #region 【任务步骤记录】
                                        _M.SetStepLog(trans, false, 1707, string.Format("[ {0} ]取砖指令超时, 尝试终止", carrier.Device.name));
                                        #endregion
                                        return;
                                    }
                                }
