...
namespace task.trans.transtask
{
    ...
    public class SameSideOutTrans : BaseTaskTrans
    {
        ...
        public override void AllocateDevice(StockTrans trans)
        {
            ...
            {
                #region 【任务步骤记录】
                _M.SetStepLog(trans, false, 1506, string.Format("等待接力库存条件；"));
                #endregion
                return;
            }
            ...
            {
                #region 【任务步骤记录】
                _M.SetStepLog(trans, false, 1106, string.Format("当前区域线内分配运输车数已达上限，等待空闲；"));
                #endregion
                return;
            }
                ...
                case TrackTypeE.后置摆渡轨道:
                        ...
                        && PubTask.Carrier.IsCarrierFree(trans.carrier_id))
                    {
                            ...
                            || mTimer.IsTimeOutAndReset(TimerTag.TileNeedCancel, trans.id, 20))
                        {
                            ...
                            #region 【任务步骤记录】
                            _M.SetStepLog(trans, false, 1206, string.Format("砖机工位非无货需求，且运输车[ {0} ]无合适轨道可以回轨；",
                                ...
                                && PubTask.Carrier.IsStopFTask(trans.carrier_id, track))
                            {
                                ...
                                if (PubTask.Carrier.IsStopFTask(trans.carrier_id, track))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            #region 【任务步骤记录】
                                            _M.SetStepLog(trans, false, 1306, string.Format("砖机[ {0} ]的工位轨道[ {1} ]不满足放砖条件；{2}；",
                        ...
                        if (isnotload)
                        {
                                   ...
                                   && PubTask.Carrier.IsStopFTask(trans.carrier_id, track))
                            {
                                ...
                                if (_M.CheckHaveCarrierInOutTrack(trans.carrier_id, trans.take_track_id, out result))
                                {
                                    ...
                                    #region 【任务步骤记录】
                                    _M.SetStepLog(trans, false, 1406, string.Format("取砖轨道[ {0} ]内有其他运输车，且运输车[ {1} ]无合适轨道可以回轨；",
                                ...
                                if (PubTask.Carrier.IsStopFTask(trans.carrier_id, track))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        //{
                                        //    cao.ToPoint = PubMaster.Track.GetTrackLimitPointOut(trans.take_track_id);
                                        //    cao.OverPoint = PubMaster.Track.GetTrackLimitPointIn(trans.take_track_id);
                                        //}
                                        //else
                                        //{
                                        //    // 去分段点取，回轨道出库地标
                                        ...
                                        ushort loc = 65535;  // 靠取砖光电：后退- 1；前进- 65535 
                                        #region 库存判断
                                        // 获取头部库存
                                        Stock takeStock = PubMaster.Goods.GetStockInLocMin(trans.take_track_id);
                                        ...
                                        if (takeStock.location > 0)
                                        {
                                            ...
                                            loc = takeStock.location;
                                        }

                                        #endregion

                                        //PubTask.Carrier.DoOrder(trans.carrier_id, trans.id, cao);
                                        MoveToTake(trans.take_track_id, trans.carrier_id, trans.id, loc);
                                        return;
                                    }
                                }
                            }
                                ...
                                if (PubTask.Carrier.IsStopFTask(trans.carrier_id, track))
                                {
                                        ...
                                        && !PubMaster.Track.IsStopUsing(trans.finish_track_id, trans.TransType))
                                    {
                                                ...
                                                || !PubMaster.Goods.IsTopStockIsGood(trans.finish_track_id, trans.goods_id))
                                        {
                                            ...
                                            //cao.CheckTra = PubMaster.Track.GetTrackUpCode(trans.finish_track_id);
                                            //cao.OverPoint = PubMaster.Track.GetTrackLimitPointIn(trans.finish_track_id);
                                        ...
                                        else
                                        {
                                            ...
                                            #region 库存判断
                                            // 获取头部库存
                                            Stock takeStock = PubMaster.Goods.GetStockInLocMin(trans.finish_track_id);
                                            if (takeStock == null || takeStock.goods_id != trans.goods_id)
                                            {
                                            ...
                                            MoveToTake(trans.finish_track_id, trans.carrier_id, trans.id, loc);
                                        }
