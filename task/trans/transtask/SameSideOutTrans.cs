...
namespace task.trans.transtask
{
    ...
    public class SameSideOutTrans : BaseTaskTrans
    {
        ...
        public override void AllocateDevice(StockTrans trans)
        {
            ...
            {
                #region 【任务步骤记录】
                _M.SetStepLog(trans, false, 1006, string.Format("存在相同作业轨道的任务，等待任务完成；"));
                #endregion
                return;
            }
            ...
            {
                #region 【任务步骤记录】
                _M.SetStepLog(trans, false, 1106, string.Format("等待接力库存条件；"));
                #endregion
                return;
            }
            ...
            {
                #region 【任务步骤记录】
                _M.SetStepLog(trans, false, 1206, string.Format("当前区域线内分配运输车数已达上限，等待空闲；"));
                #endregion
                return;
            }
            ...
            {
                #region 【任务步骤记录】
                _M.SetStepLog(trans, false, 1306, string.Format("当前没有空闲的摆渡车"));
                #endregion
                return;
            }
                    ...
                    if (isNotLoad && isStopNoOrder)
                    {
                        if (PubTask.Ferry.IsLoad(trans.take_ferry_id))
                        {
                            ...
                            if (_M.CheckHaveCarrierInTrackForUp(trans.carrier_id, trans.take_track_id, out result))
                            {
                                ...
                                #region 【任务步骤记录】
                                _M.SetStepLog(trans, false, 1506, string.Format("取砖轨道[ {0} ]内有其他运输车，且运输车[ {1} ]无合适轨道可以回轨；",
                            ...
                            {
                                // 避让判断
                                if (!PubTask.Carrier.CanDoOrderSafe(trans.carrier_id, trans.take_track_id, takeTrack.up_split_point, out result))
                                {
                                    MoveToPos(trans.take_track_id, trans.carrier_id, trans.id, CarrierPosE.轨道后侧定位点);
        ...
        public override void ToGiveTrackGiveStock(StockTrans trans)
        {
            ...
            switch (track.Type)
            {
                ...
                case TrackTypeE.下砖轨道:
                    //发送离开给上砖机
                    if (isNotLoad && !trans.IsLeaveTileLifter
                    ...
                    // 是否回轨
                    bool isback = false;
                    if (GlobalWcsDataConfig.BigConifg.IsReturnDevBackToTrack(trans.area_id, trans.line) ||  // 1.配置还车回轨
                         !PubMaster.DevConfig.IsTileNowGoodQtyOk(trans.tilelifter_id, trans.goods_id) ||        // 2.剩余上砖数量 = 0
                         (PubMaster.Track.IsEmtpy(trans.take_track_id) || PubMaster.Track.IsStopUsing(trans.take_track_id, trans.TransType)) ||     // 3.轨道状态不满足
                         _M.CheckTopStockAndSendSortTask(trans.id, trans.carrier_id, trans.take_track_id, trans.goods_id, trans.level, false)       // 4,需要接力
                         )
                    {
                        isback = true;
                    }
                ...
                case TrackTypeE.后置摆渡轨道:
                        ...
                        && mTimer.IsTimeOutAndReset(TimerTag.TileNeedCancel, trans.id, 20))
                    {
                        ...
                        #region 【任务步骤记录】
                        _M.SetStepLog(trans, false, 1406, string.Format("砖机工位非无货需求，且运输车[ {0} ]无可回轨轨道，等待砖机工位重新需求或有合适轨道可回轨；",
                            PubMaster.Device.GetDeviceName(trans.carrier_id)));
                        #endregion
                        return;
                    }
                    ...
                    if (isTileEmptyNeed && isLoad && isStopNoOrder)
                    {
                        if (PubTask.Ferry.IsLoad(trans.give_ferry_id))
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    #region 【任务步骤记录】
                                    _M.SetStepLog(trans, false, 1606, string.Format("砖机[ {0} ]的工位轨道[ {1} ]不满足放砖条件；{2}；",
                        ...
                        if (PubTask.Ferry.IsLoad(trans.give_ferry_id))
                        {
                            ...
                            if (trans.finish_track_id == 0)
                            {
                                ...
                                if (!PubMaster.Track.IsEmtpy(trans.take_track_id)
                                    && !PubMaster.Track.IsStopUsing(trans.take_track_id, trans.TransType)
                                    && !_M.CheckHaveCarrierInTrackForUp(trans.carrier_id, trans.take_track_id, out result)
                                    ...
                                    switch (type)
                                    {
                                        case DevWorkTypeE.品种作业:
                                            ...
                                            if (PubMaster.Track.HaveTrackInGoodFrist(trans.area_id, trans.tilelifter_id,
                                                ...
                                                && !_M.CheckHaveCarrierInTrackForUp(trans.carrier_id, trackid, out result)
                                                ...
                                                trans.goods_id, out List<Stock> allocatestocks))
                                            {
                                                foreach (Stock stock in allocatestocks)
                                                {
                                                    if (!_M.HaveInTrackButSortTaskForUp(stock.track_id)
                                                        ...
                                                        && !_M.CheckHaveCarrierInTrackForUp(trans.carrier_id, stock.track_id, out string _)
                            ...
                            if (trans.finish_track_id > 0 && isStopNoOrder)
                            {
                                ...
                                if ((!PubMaster.Track.IsEmtpy(trans.finish_track_id)
                                    ...
                                    || _M.CheckHaveCarrierInTrackForUp(trans.carrier_id, trans.finish_track_id, out result))
                                {
