...
namespace task.trans
{
    ...
    public class TransMaster : TransBase
    {
                ...
                case TransStatusE.取砖流程:
                    ...
                    switch (track.Type)
                    {
                        ...
                        case TrackTypeE.摆渡车_出:
                            ...
                            {
                                if (CheckHaveCarrierInOutTrack(trans.carrier_id, trans.take_track_id, out result)
                            ...
                            if (isnotload)
                            {
                                ...
                                if (PubTask.Ferry.IsLoad(trans.give_ferry_id))
                                {
                                    ...
                                    if (trans.finish_track_id == 0)
                                    {
                                        ...
                                        if (!PubMaster.Track.IsEmtpy(trans.take_track_id)
                                            && !PubMaster.Track.IsStopUsing(trans.take_track_id, trans.TransType)
                                            && !CheckHaveCarrierInOutTrack(trans.carrier_id, trans.take_track_id, out result)
        ...
        /// </summary>
        /// <param name="trans"></param>
        public void DoNewOutTrans(StockTrans trans)
        {
            ...
            switch (trans.TransStaus)
            {
                ...
                case TransStatusE.放砖流程:
                    ...
                    if (track.Type == TrackTypeE.储砖_出入 || track.Type == TrackTypeE.摆渡车_出)
                    {
                        if (trans.give_ferry_id == 0)
                        {
                            string msg = AllocateFerry(trans, DeviceTypeE.上摆渡, track, true);
                ...
                case TransStatusE.取消:
                    ...
                    if (!PubTask.TileLifter.IsTileCutover(trans.tilelifter_id)
                        ...
                        && mTimer.IsOver(TimerTag.UpTileReStoreEmtpyNeed, trans.give_track_id, 5, 5))
                    {
                        SetStatus(trans, TransStatusE.放砖流程, "砖机工位显示无货需求，恢复流程");
                        return;
                    }
                    ...
                    switch (track.Type)
                    {
                        ...
                        case TrackTypeE.上砖轨道:
                            ...
                            if (isload)
                            {
                                ...
                                {
                                    SetLoadTime(trans);
                                    SetStatus(trans, TransStatusE.放砖流程, "小车载货，恢复流程");
                                }
                            }
                            ...
                            if (trans.give_track_id == track.id)
                            {
                                ...
                                else if (PubTask.Carrier.IsStopFTask(trans.carrier_id, track))
                                {
                                    ...
                                    if (isnotload)
                                    {
                                        ...
                                        #region 【任务步骤记录】
                                        LogForCarrierSort(trans, trans.give_track_id, movecount.ToString());
                                        #endregion
                ...
                case TransStatusE.移车中:
                    ...
                    switch (track.Type)
                    {
                        ...
                        case TrackTypeE.储砖_出入:
                            ...
                            {
                                #region 【任务步骤记录】
                                LogForCarrierGive(trans, track.id);
                                #endregion
                            ...
                            if (track.id == trans.take_track_id)
                            {
                                ...
                                if (trans.take_ferry_id == 0 )
                                {
                                    ...
                                    {
                                        msg = AllocateFerry(trans, DeviceTypeE.下摆渡, track, false);
                                    }
                                    //分配上摆渡车
                                    else if(PubTask.Carrier.IsCarrierInTrackBiggerSite(trans.carrier_id, track.id)
                                        && !PubTask.Carrier.ExistCarInFront(trans.carrier_id, track.id))
                                    {
                                        msg = AllocateFerry(trans, DeviceTypeE.上摆渡, track, false);
                                    }
