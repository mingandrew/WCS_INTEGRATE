...
namespace task.trans
{
    public abstract class TransBase
    {
        ...
        internal void SetCarrier(StockTrans trans, uint carrierid, string memo = "")
        {
            ...
            {
                mLog.Status(true, string.Format("任务[ {0} ], 分配小车[ {1} ], 备注[ {2} ]", trans.id, PubMaster.Device.GetDeviceName(carrierid), memo));
        ...
        internal void SetTile(StockTrans trans, uint tileid, string memo = "")
        {
            ...
            {
                mLog.Status(true, string.Format("任务[ {0} ], 重新分配砖机[ {1} ], 备注[ {2} ]", trans.id, PubMaster.Device.GetDeviceName(tileid), memo));
