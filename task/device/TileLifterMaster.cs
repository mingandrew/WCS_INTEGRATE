            ...
            else
            {
                ...
                if (mTimer.IsOver(TimerTag.UpTileHaveNotStockToOut, tileid, Site_1, PubMaster.Dic.GetDtlIntCode(DicTag.UpTileNoStockWaitTime), 10) && PubMaster.DevConfig.GetTileAutoShiftGood(tileid))
                {
                    ...
                    if (_pregoodsid == 0 && PubMaster.Goods.IsHavePreStockGood(tileid))
                    {
                        ...
                        PreStockGood firstgood = PubMaster.Goods.GetNextPreStockGood(tileid, goodid, level);
                        if (firstgood != null)
                        {
                            PubMaster.DevConfig.UpdateTilePreGood(tileid, goodid, firstgood.good_id, (firstgood.pre_good_all ? 0 : firstgood.pre_good_qty), firstgood.level, out string mes);
                            _pregoodsid = PubMaster.DevConfig.GetDevicePreId(tileid);
                        }
                    }
                    if (_pregoodsid != 0)
                    {
                        PubMaster.DevConfig.UpdateShiftTileGood(tileid, goodid, out string msg);
                        PubMaster.DevConfig.AddLog(string.Format("上砖机找不到库存可上满{0}秒，自动转产", PubMaster.Dic.GetDtlIntCode(DicTag.UpTileNoStockWaitTime)));
                    }
                }
            }
