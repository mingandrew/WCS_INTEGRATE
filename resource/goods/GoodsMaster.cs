...
namespace resource.goods
{
    public class GoodsMaster
    {
        ...
        /// <returns></returns>
        public PreStockGood GetNextPreStockGood(uint tileid, uint goodid, byte level)
        {
            ...
            if (plist != null && plist.Count != 0)
            {
                switch (tilePre)
                {
                    ...
                    case TilePreGoodType.循环预设列表:
                        int order = plist.Find(c => c.EqualGoodAndLevel(goodid, level))?.order ?? 0;
            ...
            {
                StockSum nextg = PubMaster.Sums.GetFirstPreGood(tileid);
                if (nextg == null)
                {
                    return null;
                }
                ...
                {
                    tile_id = tileid,
                    good_id = nextg.goods_id,
                    order = 1,
                    pre_good_all = true,
                    pre_good_qty = 0,
                    level = nextg.sum_level,
                };
            }
            return null;
        }
