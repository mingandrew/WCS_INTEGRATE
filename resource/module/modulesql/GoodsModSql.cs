...
namespace resource.module.modulesql
{
    public class GoodsModSql : BaseModSql
    {
        ...
        {
            string str = "INSERT INTO `pre_stock_goods`(`tile_id`, `good_id`, `order`, `pre_good_qty`, `pre_good_all`, `level`) VALUES ({0}, {1}, {2}, {3}, {4}, {5})";
            string sql = string.Format(@str, preStock.tile_id, preStock.good_id, preStock.order, preStock.pre_good_qty, preStock.pre_good_all, preStock.level);
            int row = mSql.ExcuteSql(sql);
            return row >= 1;
        }
        #endregion
