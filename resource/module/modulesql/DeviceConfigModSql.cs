        ...
        internal bool EditConfigTileLifter(ConfigTileLifter dev, TileConfigUpdateE type)
        {
            ...
            switch (type)
            {
                case TileConfigUpdateE.Goods:
                    sql += string.Format("old_goodid = {0}, goods_id = {1}, pre_goodid = {2}, do_shift = {3} , `now_good_qty` = '{4}', `pre_good_qty` = '{5}', `now_good_all` = {6}, `pre_good_all` = {7} ,`pre_level` = {8} ,`level` = {9}",
                        dev.old_goodid, GetIntOrNull(dev.goods_id), dev.pre_goodid, dev.do_shift, dev.now_good_qty, dev.pre_good_qty, dev.now_good_all, dev.pre_good_all, dev.pre_level, dev.level);
                    break;
